  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        preview.srcObject = stream; preview.setAttribute("playsinline", "true"); preview.muted = true;
        preview.style.cssText = "position:fixed;bottom:80px;left:16px;width:160px;height:120px;border-radius:12px;border:2px solid #00aaff44;z-index:999;object-fit:cover;opacity:0.85;pointer-events:none;";
        document.body.appendChild(preview); preview.play(); previewRef.current = preview;
          ...
          try {
            if (video.readyState >= 2) {
              ...
              if (result.landmarks && result.landmarks.length > 0) {
                ...
                handRef.current = { active: true, x: lm[0].x, y: lm[0].y, pinch: pinchDist };
              } else { handRef.current = { ...handRef.current, active: false }; }
            }
          } catch (e) { /* ignore */ }
          rafRef.current = requestAnimationFrame(detect);
        };
        detect();
      } catch (err) { console.warn("Hand tracking failed:", err); if (handRef.current) handRef.current.active = false; }
    })();
    return () => {
      cancelled = true;
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
...
/* ══════════════════════════════════════════════════════════
   DNA HELIX
══════════════════════════════════════════════════════════ */
function DNAHelix({ onNodeClick, rotRef, onContactClick, handRef }) {
  const grpRef = useRef();
  ...
  useFrame((_, dt) => {
    ...
    if (handRef && handRef.current && handRef.current.active) {
      ...
      const currentY = grpRef.current.rotation.y;
      grpRef.current.rotation.y += (targetY - currentY) * 0.06;
    }
    const BASE_X = 0.12;
    if (rotRef) {
...
function ParticleSystem({ handRef }) {
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i < PART_COUNT; i++) {
      ...
      const home = new THREE.Vector3(Math.cos(a) * r, y, Math.sin(a) * r);
      arr.push({
        home: home.clone(), pos: home.clone(),
        vel: new THREE.Vector3(), phase: Math.random() * Math.PI * 2,
        speed: 0.5 + Math.random() * 2, size: 0.04 + Math.random() * 0.1,
      });
    }
    return arr;
  }, []);
  useFrame(({ clock }) => {
    if (!meshRef.current) return;
    const t = clock.getElapsedTime();
    const active = handRef?.current?.active || false;
    const pinch = handRef?.current?.pinch ?? 1;
    for (let i = 0; i < PART_COUNT; i++) {
      const p = data[i];
      const { pos, vel, home } = p;
      ...
      pos.add(vel);
      const sc = p.size * (0.6 + 0.4 * Math.sin(t * p.speed + p.phase));
...
function HandStatus({ handRef }) {
  ...
  return (
    <div style={{
      ...
      background: "rgba(0,20,40,0.8)", backdropFilter: "blur(8px)",
      border: "1px solid #00aaff55", borderRadius: 10, padding: "8px 14px",
...
export default function App() {
  ...
  return (<>
    ...
    >
      ...
      <Lights />
      <DNAHelix onNodeClick={handleNodeClick} rotRef={rotRef} onContactClick={() => setContactCard(true)} handRef={handRef} />
      <MatrixRain />
      <SparkleSwarm />
